(function(){
  ...
  function loadNav(){if(document.querySelector('script[data-rrta-platform-nav]'))return;const s=document.createElement('script');s.src='trainer-navigation.js?v=20260917-3';s.dataset.rrtaPlatformNav='1';document.head.appendChild(s)}
  function boot(){
    document.body.dataset.portalNav='academy';
    const existing=document.querySelector('aside.side,aside.booking-side,.shell > aside.sidebar,.portal-shell > aside.sidebar');
    if(existing){loadNav();return;}
    const direct=[...document.body.children].filter(el=>!['SCRIPT','STYLE'].includes(el.tagName));
    const wrapper=document.createElement('div');wrapper.className='rrta-generated-shell shell';
    const side=document.createElement('aside');side.className='side rrta-generated-side';
    side.innerHTML='<div class="brand"></div><div class="spacer"></div>';
