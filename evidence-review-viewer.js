(function(){
  ...
  async function loadReceipts(){
    ...
    const {data,error}=await db.from('evidence_review_requests').select('id,qualification_name,reviewer_note,reviewed_at,learner_read_at,profiles:learner_id(full_name)').eq('status','rejected').order('reviewed_at',{ascending:false}).limit(20);
    if(error)return;
    ...
    panel.innerHTML=`<div style="font-size:10px;color:#b20f22;font-weight:900;text-transform:uppercase;letter-spacing:.11em">Returned Evidence</div><h2>Read Receipts</h2><p style="margin:0;color:#6f7a88;font-size:11px">Shows whether the learner has opened the feedback you sent with a rejected submission.</p><div class="rrta-receipt-list">${(data||[]).length?(data||[]).map(r=>`<div class="rrta-receipt-item"><div><strong>${esc(r.profiles?.full_name||'Learner')} · ${esc(r.qualification_name||'Qualification')}</strong><p>${esc(r.reviewer_note||'No reviewer note')}</p><p>Returned ${esc(fmtDateTime(r.reviewed_at))}</p></div><span class="rrta-read-pill ${r.learner_read_at?'read':'unread'}">${r.learner_read_at?'✓ Read '+esc(fmtDateTime(r.learner_read_at)):'Not read yet'}</span></div>`).join(''):'<div style="padding:18px;text-align:center;color:#6f7a88;font-size:11px">No returned evidence yet.</div>'}</div>`;
