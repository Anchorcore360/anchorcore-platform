(function(){
  ...
  async function complianceSummary(person){
    ...
    let current=0,missing=0,expiring=0;for(const r of reqs||[]){const a=rrtaCompliancePick(r.compliance_items?.name,accs,x=>match(r.compliance_items?.name,x.accreditation_name)),st=statusFor(a);if(st.label==='Valid')current++;else if(st.label==='Expiring'){current++;expiring++}else missing++}
