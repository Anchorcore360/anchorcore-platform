...
const COURSE_ID = "customer-service";
...
const DEMO_USERS = [
  {id:"trainer-1", role:"trainer", name:"RRTA Trainer", email:"trainer@rrta.demo", password:"Trainer123!"},
  {id:"learner-1", role:"learner", name:"John Smith", email:"john.smith@rrta.demo", password:"Learner123!", company:"Rapid Response Telecoms"},
  {id:"learner-2", role:"learner", name:"Sarah Jones", email:"sarah.jones@rrta.demo", password:"Learner123!", company:"Rapid Response Telecoms"},
  {id:"learner-3", role:"learner", name:"Mike Brown", email:"mike.brown@rrta.demo", password:"Learner123!", company:"Rapid Response Telecoms"}
];

const $ = id => document.getElementById(id);
let session = null, exam=[], answers=[], qIndex=0, secondsLeft=DURATION, timer=null, lastResult=null;

function defaultState(){
  return {
    assignments: {
      "learner-1": { [COURSE_ID]: {status:"assigned", assignedAt:new Date().toISOString()} },
      "learner-2": {},
      "learner-3": {}
    },
    results: []
...
function loadState(){
  try{
    const x=JSON.parse(localStorage.getItem("rrtaPortalState"));
    return x && x.assignments && x.results ? x : defaultState();
  }catch(e){ return defaultState(); }
}
let state=loadState();
function saveState(){ localStorage.setItem("rrtaPortalState",JSON.stringify(state)); }
...
function fmtDate(s){ return s ? new Intl.DateTimeFormat("en-GB").format(new Date(s)) : "—"; }
function learner(id){ return DEMO_USERS.find(u=>u.id===id); }
function latestResult(learnerId){
  return [...state.results].reverse().find(r=>r.learnerId===learnerId && r.courseId===COURSE_ID) || null;
}
function assigned(learnerId){ return Boolean(state.assignments[learnerId]?.[COURSE_ID]); }

function login(user){
  session=user;
  $("headerUser").classList.remove("hidden");
  $("headerUser").textContent=`${user.name} · ${user.role==="trainer"?"Trainer / Assessor":"Learner"}`;
  if(user.role==="trainer"){ renderTrainer(); showScreen("trainer"); }
  else { renderLearner(); showScreen("learner"); }
}
function logout(){
  clearInterval(timer);
  session=null;
...
$("loginForm").onsubmit=e=>{
  ...
  $("loginError").classList.add("hidden"); login(user);
};
document.querySelector('[data-demo="trainer"]').onclick=()=>{$("loginEmail").value="trainer@rrta.demo";$("loginPassword").value="Trainer123!";};
document.querySelector('[data-demo="learner"]').onclick=()=>{$("loginEmail").value="john.smith@rrta.demo";$("loginPassword").value="Learner123!";};
...
function trainerView(name){
  ...
  document.querySelectorAll("[data-trainer-view]").forEach(b=>b.classList.toggle("active",b.dataset.trainerView===name));
  if(name==="learners") renderLearnersTable();
  if(name==="results") renderResultsTable();
}
document.querySelectorAll("[data-trainer-view]").forEach(b=>b.onclick=()=>trainerView(b.dataset.trainerView));

function renderTrainer(){
  const learners=DEMO_USERS.filter(u=>u.role==="learner");
  const assignedCount=learners.filter(u=>assigned(u.id)).length;
  const passed=learners.filter(u=>latestResult(u.id)?.passed).length;
  const attempts=state.results.length;
  $("trainerStats").innerHTML=`
    <div class="stat"><span>Learners</span><strong>${learners.length}</strong></div>
    <div class="stat alert"><span>Assigned learning</span><strong>${assignedCount}</strong></div>
    <div class="stat"><span>Passed</span><strong>${passed}</strong></div>
    <div class="stat"><span>Assessment attempts</span><strong>${attempts}</strong></div>`;
  const recent=[...state.results].reverse().slice(0,5);
  $("recentActivity").innerHTML=recent.length?recent.map(r=>`
    <div class="activity"><span class="dot"></span><div><strong>${esc(learner(r.learnerId)?.name||"Learner")}</strong>
    <small>${COURSE_NAME} · ${r.passed?"PASS":"REFER"} · ${r.score}/20 · ${fmtDate(r.completedAt)}</small></div></div>`).join("")
    : `<p class="muted">No assessment activity yet. Complete an assessment as a learner and it will appear here.</p>`;
  renderLearnersTable(); renderResultsTable(); populateAssign();
}
function learnerStatus(u){
  const r=latestResult(u.id);
  if(r) return r.passed?`<span class="pill pass">Passed ${r.score}/20</span>`:`<span class="pill refer">Referred ${r.score}/20</span>`;
  if(assigned(u.id)) return `<span class="pill assigned">Assigned</span>`;
  return `<span class="pill none">Not assigned</span>`;
}
function renderLearnersTable(){
  const users=DEMO_USERS.filter(u=>u.role==="learner");
  $("learnersTable").innerHTML=`<div class="table-wrap"><table class="table"><thead><tr><th>Learner</th><th>Company</th><th>Customer Service</th><th>Actions</th></tr></thead><tbody>
  ${users.map(u=>`<tr><td><strong>${esc(u.name)}</strong><br><small>${esc(u.email)}</small></td><td>${esc(u.company)}</td><td>${learnerStatus(u)}</td>
  <td><div class="row-actions">${assigned(u.id)?`<button class="mini-btn red" onclick="removeAssignment('${u.id}')">Remove</button>`:`<button class="mini-btn" onclick="assignCourse('${u.id}')">Assign course</button>`}</div></td></tr>`).join("")}
  </tbody></table></div>`;
}
function renderResultsTable(){
  const rows=[...state.results].reverse();
  $("resultsTable").innerHTML=rows.length?`<div class="table-wrap"><table class="table"><thead><tr><th>Learner</th><th>Course</th><th>Date</th><th>Score</th><th>Result</th></tr></thead><tbody>
  ${rows.map(r=>`<tr><td>${esc(learner(r.learnerId)?.name||r.learnerId)}</td><td>${COURSE_NAME}</td><td>${fmtDate(r.completedAt)}</td><td>${r.score}/20 (${r.pct}%)</td><td><span class="pill ${r.passed?"pass":"refer"}">${r.passed?"PASS":"REFER"}</span></td></tr>`).join("")}
  </tbody></table></div>`:`<p class="muted">No results recorded yet.</p>`;
}
window.assignCourse=id=>{
  state.assignments[id] ||= {};
  state.assignments[id][COURSE_ID]={status:"assigned",assignedAt:new Date().toISOString()};
  saveState(); renderTrainer();
};
window.removeAssignment=id=>{
  if(state.assignments[id]) delete state.assignments[id][COURSE_ID];
  saveState(); renderTrainer();
};
function populateAssign(){
  $("assignLearner").innerHTML=DEMO_USERS.filter(u=>u.role==="learner").map(u=>`<option value="${u.id}">${esc(u.name)}</option>`).join("");
}
function openAssign(){ populateAssign(); $("assignModal").classList.remove("hidden"); }
function closeAssign(){ $("assignModal").classList.add("hidden"); }
$("quickAssignBtn").onclick=openAssign; $("closeAssignModal").onclick=closeAssign;
$("confirmAssign").onclick=()=>{ assignCourse($("assignLearner").value); closeAssign(); trainerView("learners"); };
$("demoAddLearnerBtn").onclick=()=>alert("The real version will create a user account here. This prototype includes three demo learners.");

function renderLearner(){
  $("learnerWelcome").textContent=`Welcome, ${session.name}`;
  const isAssigned=assigned(session.id), r=latestResult(session.id);
  if(!isAssigned){
    $("learnerDashboard").innerHTML=`<div class="empty-state"><div class="empty-icon">✓</div><h2>No assessments assigned</h2><p class="muted">You currently have no learning or assessments to complete. New assignments from your trainer will appear here.</p></div>`;
    return;
  }
  let status=r?(r.passed?"Completed":"Reassessment required"):"Assigned";
  $("learnerDashboard").innerHTML=`<div class="learning-grid"><article class="learning-card">
    <span class="pill ${r?(r.passed?"pass":"refer"):"assigned"}">${status}</span>
    <h2>${COURSE_NAME}</h2>
    <p class="muted">Face-to-face course for field engineers with an online summative assessment.</p>
    <div class="learning-meta"><span>Face-to-face</span><span>20-minute assessment</span><span>80% pass mark</span></div>
    ${r?`<p><strong>Latest result:</strong> ${r.score}/20 (${r.pct}%) — ${r.passed?"PASS":"REFER"}</p>`:""}
    <button class="btn primary" id="openCourseBtn">${r&&r.passed?"View course":"Open learning"}</button>
  </article></div>`;
  $("openCourseBtn").onclick=()=>{ renderCourse(); showScreen("course"); };
}
function renderCourse(){
  const r=latestResult(session.id);
  let html="";
  if(r?.passed){
    html=`<div class="course-action"><span class="pill pass">Assessment passed</span><h3>Completed</h3><p>You achieved ${r.score}/20 (${r.pct}%). Your latest result is saved to your learning record in this demonstration.</p>
    <button class="btn secondary" id="retakeDemo">Retake for demo</button></div>`;
  } else if(r && !r.passed){
    html=`<div class="course-action"><span class="pill refer">Reassessment required</span><h3>Speak to your trainer / assessor</h3><p>Your latest score was ${r.score}/20. Further teaching should take place before reassessment.</p><button class="btn primary" id="startCourseAssessment">Start reassessment</button></div>`;
  } else {
    html=`<div class="course-action"><h3>Your assessment is ready</h3><p>Begin only when instructed by your trainer / assessor.</p><button class="btn primary" id="startCourseAssessment">Start assessment</button></div>`;
  }
  $("courseAction").innerHTML=html;
  const start=$("startCourseAssessment"); if(start) start.onclick=assessmentIntro;
  const retake=$("retakeDemo"); if(retake) retake.onclick=assessmentIntro;
}
$("backToLearning").onclick=()=>{renderLearner();showScreen("learner");};

function assessmentIntro(){
  $("assessmentDeclaration").checked=false; $("beginAssessment").disabled=true; showScreen("assessment-intro");
}
$("assessmentDeclaration").onchange=e=>$("beginAssessment").disabled=!e.target.checked;
$("cancelAssessment").onclick=()=>{renderCourse();showScreen("course");};
...
function shuffle(a){
  a=[...a]; for(let i=a.length-1;i>0;i--){const j=secureRandom(i+1);[a[i],a[j]]=[a[j],a[i]];} return a;
}
function buildExam(){
  let chosen=[];
  Object.entries(BLUEPRINT).forEach(([cat,n])=>{
    chosen.push(...shuffle(QUESTIONS.filter(q=>q.category===cat)).slice(0,n));
  });
...
function startAssessment(){
  exam=buildExam(); answers=Array(20).fill(null); qIndex=0; secondsLeft=DURATION; lastResult=null; showScreen("test"); renderQuestion(); startTimer();
}
function renderQuestion(){
  const q=exam[qIndex];
  $("progressText").textContent=`Question ${qIndex+1} of 20`;
  $("progressBar").style.width=`${((qIndex+1)/20)*100}%`;
  $("questionText").textContent=q.q;
  $("scenarioImageWrap").classList.add("hidden");
  if(q.image){
    $("scenarioImage").src=q.image;
    $("scenarioImage").onload=()=>$("scenarioImageWrap").classList.remove("hidden");
    $("scenarioImage").onerror=()=>$("scenarioImageWrap").classList.add("hidden");
  }
  $("answerList").innerHTML=q.options.map((o,i)=>`<button class="answer ${answers[qIndex]===i?"selected":""}" data-i="${i}"><span class="answer-letter">${String.fromCharCode(65+i)}</span><span>${esc(o.text)}</span></button>`).join("");
  document.querySelectorAll(".answer").forEach(b=>b.onclick=()=>{answers[qIndex]=Number(b.dataset.i);renderQuestion();});
  $("prevQuestion").disabled=qIndex===0;
  $("nextQuestion").textContent=qIndex===19?"Review answers":"Next";
}
$("prevQuestion").onclick=()=>{if(qIndex>0){qIndex--;renderQuestion();}};
$("nextQuestion").onclick=()=>{if(qIndex<19){qIndex++;renderQuestion();}else renderReview();};
function startTimer(){
  clearInterval(timer); updateTimer();
  timer=setInterval(()=>{secondsLeft--;updateTimer();if(secondsLeft<=0){clearInterval(timer);submitNow(true);}},1000);
}
...
function renderReview(){
  ...
  $("reviewSummary").textContent=`${count} of 20 questions answered. You must answer all questions before submitting.`;
  $("reviewGrid").innerHTML=answers.map((a,i)=>`<button class="review-q ${a!==null?"answered":""}" data-i="${i}">${i+1}</button>`).join("");
  document.querySelectorAll(".review-q").forEach(b=>b.onclick=()=>{qIndex=Number(b.dataset.i);showScreen("test");renderQuestion();});
  $("submitAssessment").disabled=count!==20;
}
$("returnToQuestion").onclick=()=>{showScreen("test");renderQuestion();};
$("submitAssessment").onclick=()=>{if(confirm("Submit your assessment? You will not be able to change your answers afterwards.")) submitNow(false);};

function submitNow(timedOut){
  clearInterval(timer);
  let score=0; exam.forEach((q,i)=>{if(answers[i]!==null && q.options[answers[i]]?.correct)score++;});
  const pct=Math.round(score/20*100), passed=score>=PASS_MARK;
  const result={
    id:"result-"+Date.now(), learnerId:session.id, courseId:COURSE_ID, score,pct,passed,timedOut,
    completedAt:new Date().toISOString(), questionIds:exam.map(q=>q.id)
  };
  state.results.push(result); saveState(); lastResult=result; renderResult(); showScreen("result");
}
function renderResult(){
  $("resultCard").classList.toggle("refer",!lastResult.passed);
  $("resultBadge").textContent=lastResult.passed?"PASS":"REFER";
  $("resultHeading").textContent=lastResult.passed?"Assessment passed":"Assessment referred";
  $("resultLearner").textContent=session.name;
  $("resultScore").textContent=`${lastResult.score}/20 · ${lastResult.pct}%`;
  $("resultMessage").textContent=lastResult.passed?"You have achieved the required 80% standard. Your result has been added to your learning record.":"The required pass mark is 16/20. Please speak to your trainer / assessor before reassessment.";
  $("downloadCertificate").style.display=lastResult.passed?"inline-block":"none";
}
$("finishResult").onclick=()=>{renderLearner();showScreen("learner");};

async function logoData(){
  return new Promise((resolve,reject)=>{
    const img=new Image();img.onload=()=>{const c=document.createElement("canvas");c.width=img.naturalWidth;c.height=img.naturalHeight;c.getContext("2d").drawImage(img,0,0);resolve(c.toDataURL("image/png"));};img.onerror=reject;img.src="rrta-logo.png";
  });
}
function filename(s){return s.replace(/[^a-z0-9]+/gi,"_").replace(/^_|_$/g,"");}
$("downloadCertificate").onclick=async()=>{
  if(!window.jspdf){alert("PDF library unavailable. Please check your internet connection.");return;}
  const {jsPDF}=window.jspdf,doc=new jsPDF({orientation:"landscape",unit:"mm",format:"a4"}),W=297,H=210;
  doc.setDrawColor(157,11,26);doc.setLineWidth(1.8);doc.rect(8,8,W-16,H-16);doc.setDrawColor(38,43,48);doc.setLineWidth(.4);doc.rect(11,11,W-22,H-22);
  try{doc.addImage(await logoData(),"PNG",106,18,85,31);}catch(e){}
  doc.setTextColor(157,11,26);doc.setFont("helvetica","bold");doc.setFontSize(13);doc.text("CERTIFICATE OF ACHIEVEMENT",W/2,61,{align:"center"});
  doc.setTextColor(38,43,48);doc.setFont("helvetica","normal");doc.setFontSize(11);doc.text("This is to certify that",W/2,75,{align:"center"});
  doc.setFont("helvetica","bold");doc.setFontSize(24);doc.text(session.name,W/2,91,{align:"center"});
  doc.setDrawColor(185,188,191);doc.line(80,96,217,96);doc.setFont("helvetica","normal");doc.setFontSize(11);doc.text("has successfully completed the",W/2,108,{align:"center"});
  doc.setTextColor(157,11,26);doc.setFont("helvetica","bold");doc.setFontSize(19);doc.text(COURSE_NAME,W/2,121,{align:"center"});
  doc.setTextColor(82,88,94);doc.setFont("helvetica","normal");doc.setFontSize(10);doc.text("Face-to-Face Course for Field Engineers",W/2,131,{align:"center"});
  const d=fmtDate(lastResult.completedAt);doc.setFillColor(244,245,246);doc.roundedRect(37,148,223,25,3,3,"F");
  [["DATE COMPLETED",d],["ASSESSMENT RESULT",`${lastResult.score}/20 (${lastResult.pct}%)`],["CERTIFICATE NO.",`RRTA-CS-${String(lastResult.id).slice(-8)}`]].forEach(([k,v],i)=>{const x=[74,148.5,223][i];doc.setTextColor(105,112,118);doc.setFontSize(7);doc.setFont("helvetica","bold");doc.text(k,x,157,{align:"center"});doc.setTextColor(38,43,48);doc.setFontSize(10);doc.text(v,x,166,{align:"center"});});
  doc.setTextColor(157,11,26);doc.setFontSize(7.5);doc.text("RAPID RESPONSE TRAINING ACADEMY",W/2,195,{align:"center"});
  doc.save(`${filename(session.name)}_Customer_Service_Certificate.pdf`);
};
$("downloadRecord").onclick=async()=>{
  if(!window.jspdf){alert("PDF library unavailable. Please check your internet connection.");return;}
  const {jsPDF}=window.jspdf,doc=new jsPDF({unit:"mm",format:"a4"});try{doc.addImage(await logoData(),"PNG",65,12,80,29);}catch(e){}
  doc.setTextColor(157,11,26);doc.setFont("helvetica","bold");doc.setFontSize(15);doc.text("ASSESSMENT RECORD",105,51,{align:"center"});
  doc.setTextColor(38,43,48);doc.setFontSize(14);doc.text(COURSE_NAME,105,61,{align:"center"});doc.setDrawColor(210);doc.line(18,68,192,68);
  const rows=[["Learner",session.name],["Company",session.company||""],["Date",fmtDate(lastResult.completedAt)],["Score",`${lastResult.score}/20 (${lastResult.pct}%)`],["Result",lastResult.passed?"PASS":"REFER"],["Question Bank IDs",lastResult.questionIds.join(", ")]];
  let y=80;rows.forEach(([k,v])=>{doc.setFontSize(9);doc.setFont("helvetica","bold");doc.text(k,24,y);doc.setFont("helvetica","normal");const lines=doc.splitTextToSize(String(v),112);doc.text(lines,73,y);y+=Math.max(9,lines.length*5);});
  doc.save(`${filename(session.name)}_Assessment_Record.pdf`);
};
