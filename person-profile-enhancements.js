(function(){
  ...
  function editableControl(label,value){
    ...
    if(label==='System access'){const current=String(value||'operative').toLowerCase();const roles=[['operative','Operative — Own Profile & Learning'],['supervisor','Supervisor — Operatives In Their Team'],['manager','Manager — Supervisors & Their Teams'],['rrt_subcontractor_operator','RRT Subcontractor Operator'],['rrt_subcontractor_manager','RRT Subcontractor Manager'],['external','External — No Access'],['health_safety','Health & Safety'],['operations_manager','Operations Manager — Managers & Their Teams'],['administrator','Administrator'],['academy_staff','Academy Staff — Full Academy Access']];return '<select data-profile-field="system_role" style="'+fieldStyle+'">'+roles.map(([v,l])=>'<option value="'+v+'" '+(current===v||(current==='academy'&&v==='academy_staff')?'selected':'')+'>'+l+'</option>').join('')+'</select>'}
