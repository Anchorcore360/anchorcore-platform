(function(){
  ...
  function editableControl(label,value){
    ...
    const map={'Forename':['forename','text'],'Surname':['surname','text'],'Email':['email','email'],'Employee number':['employee_number','text'],'Date of birth':['date_of_birth','date'],'Job title':['job_title','text'],'Organisation':['organisation','text']};
  ...
  function enterInlineEdit(){
    ...
    grid.querySelectorAll('.detail').forEach(card=>{const label=(card.querySelector('span')?.textContent||'').trim();const control=editableControl(label,values[label]);if(control){const value=card.querySelector('strong');if(value)value.outerHTML=control}});
  ...
  async function saveInlineEdit(){
    ...
    learner=data;msg.textContent='Saved';msg.style.color='#176b37';await render();
  }
