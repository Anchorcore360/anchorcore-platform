(function(){
  const accessLabel=v=>({operative:'Operative',supervisor:'Supervisor',manager:'Manager',operations_manager:'Operations Manager',academy:'Academy'}[String(v||'').toLowerCase()]||'Operative');
  ...
  function editableControl(label,value){
    ...
    if(label==='System access'){const current=String(value||'operative');return '<select data-profile-field="system_role" style="'+fieldStyle+'"><option value="operative" '+(current==='operative'?'selected':'')+'>Operative</option><option value="supervisor" '+(current==='supervisor'?'selected':'')+'>Supervisor</option><option value="manager" '+(current==='manager'?'selected':'')+'>Manager</option><option value="operations_manager" '+(current==='operations_manager'?'selected':'')+'>Operations Manager</option><option value="academy" '+(current==='academy'?'selected':'')+'>Academy</option></select>'}
