(function(){
  ...
  function ensureBookedTab(){
    ...
    if(!tabs.querySelector('[data-tab="bookings"]')){
      const b=document.createElement('button');b.className='tab-btn';b.dataset.tab='bookings';b.textContent='Booked Courses';overviewBtn.insertAdjacentElement('afterend',b);
    }
    if(!tabs.querySelector('[data-tab="compliance"]')){
      const b=document.createElement('button');b.className='tab-btn';b.dataset.tab='compliance';b.textContent='Compliance';const acc=tabs.querySelector('[data-tab="accreditations"]');if(acc)acc.insertAdjacentElement('beforebegin',b);else tabs.appendChild(b);
    }
  ...
  function ensurePanels(){
    ...
    if(!document.getElementById('tab-compliance')){
      const p=document.createElement('section');p.id='tab-compliance';p.className='tab-panel';p.innerHTML='<div id="complianceProfileHost"></div>';document.getElementById('tab-accreditations')?.insertAdjacentElement('beforebegin',p);
    }
  }

  function setMetric(id,label,value){
    const valueEl=document.getElementById(id);if(!valueEl)return;
    const card=valueEl.closest('.metric');const labelEl=card?.querySelector('span');if(labelEl)labelEl.textContent=label;valueEl.textContent=value;
  }
  ...
  function renderLearningHome(){
    ...
    setMetric('statAccreditations','Upcoming Bookings',upcoming);
    setMetric('statAssignments','Assigned Learning',assignedOpen);
    setMetric('statAttempts','Completed Assessments',typeof attempts!=='undefined'?attempts.length:0);
    setMetric('statCertificates','Accreditations',typeof accreditations!=='undefined'?accreditations.length:0);
    let learning=document.getElementById('learningHome');
    if(!learning){learning=document.createElement('div');learning.id='learningHome';learning.className='two-col';if(stats)stats.insertAdjacentElement('afterend',learning);else overview.prepend(learning);}
  ...
  function moveComplianceManagement(){
    ...
    const manager=document.getElementById('managerPanel'),roles=document.getElementById('jobRolePanel'),snapshot=document.getElementById('complianceSnapshot');
    ...
    if(!snapshotCard&&snapshot){snapshotCard=snapshot.closest('.panel-card');if(snapshotCard){snapshotCard.id='profileComplianceCard';const h=snapshotCard.querySelector('h2');if(h)h.textContent='Compliance Requirements';}}
  ...
  function mergeCertificatesIntoAccreditations(){
    ...
    if(!certBlock){certBlock=document.createElement('article');certBlock.id='profileCertificateBlock';certBlock.className='panel-card';certBlock.style.marginTop='16px';certBlock.innerHTML='<div class="panel-head-row"><div><h2>Course Certificates</h2><p class="muted">Certificates created from completed course activity.</p></div></div><div id="profileCertificateList" class="record-list"></div>';panel.appendChild(certBlock);}
    const source=document.getElementById('certificateList'),target=document.getElementById('profileCertificateList');if(source&&target)target.innerHTML=source.innerHTML;
  }
  ...
  if(typeof render==='function'){const prior=render;render=async function(){const r=await prior.apply(this,arguments);setTimeout(apply,80);return r};}
})();
